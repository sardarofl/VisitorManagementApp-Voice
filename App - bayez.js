...
export default function App() {
  ...
  const triggerPhrase = "Hey MyLobby"; // Define your trigger phrase
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      Voice.destroy().then(() => {
        ...
        Voice = null; // Reset the Voice instance to null
      });    };
  }, []);
  ...
  const startVoiceListener = async () => {
    try {
      if (!Voice) {
        // Reinitialize Voice if it's null
  ...
  const onSpeechResultsHandler = async (event) => {
    ...
    setSilenceTimer(setTimeout(() => {
      ...
      stopRecording();
    }, 3000)); // 3 seconds of silence to stop recording
  
    if (event.value && event.value.some(v => v.toLowerCase().includes(triggerPhrase.toLowerCase()))) {
  ...
  const stopRecording = async () => {
    ...
    try { 
      ...
      console.log('Recording finished', uri); // Log with URI
  
      if (uri) {
        handleTranscription(uri);
      } else {
        console.error('No recording file found');
        setIsProcessing(false); // Stop processing if no recording is found
      }
    } catch (error) {
    ...
    <PaperProvider>
      <View style={styles.container}>
        ...
        <PaperButton
          icon="record"
          mode="elevated"
          onPressIn={() => {  /* Removed startRecording() call */ }}
